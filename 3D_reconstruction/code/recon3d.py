...
def reconstruct3D(transform_candidates, calibrated_1, calibrated_2):
  ...
  for candidate in transform_candidates:
    ...
    # print(calibrated_1.shape) # n x 3
    # print(calibrated_2.shape)

    for i in range(calibrated_1.shape[0]):
        x1 = calibrated_1[i]
        x2 = calibrated_2[i]
        
        # Construct the A matrix and b vector for Ax = b
        A = np.zeros((3, 2))
        A[:, 0] = x2.T
        A[:, 1] = (-R @ x1).T
        b = T
        
        # Solve for lambdas using least squares
        lambdas[:, i], _, _, _ = np.linalg.lstsq(A, b, rcond=None)
